 ...
 */
abstract class P2_List_Creator {
	/**
...
jQuery( function( $ ) {
	$( 'body' )
		...
		.delegate( '.p2-task-list :checkbox', 'click', function() {
			...
			while ( colorEl.prop( 'tagname' ) && colorEl.css( 'background-color' ).match( /^\s*(rgba\s*\(\s*0+\s*,\s*0+\s*,\s*0+\s*,\s*0+\s*\)|transparent)\s*$/ ) ) {
				colorEl = colorEl.parent();
			}

			color = colorEl.prop( 'tagname' ) ? colorEl.css( 'background-color' ) : '#ffffff';
