<?php
  ...
  function create_post_type() {
    ...
    register_post_type( 'climb',
     array(
      'labels' => array(
        'name' => __( 'Climbs' ),
        'singular_name' => __( 'Climb' )
       ),
      'public' => true,
      'has_archive' => true
      ));
    register_post_type( 'drive',
     array(
      'labels' => array(
        'name' => __( 'Drives' ),
        'singular_name' => __( 'Drive' )
       ),
      'public' => true,
      'has_archive' => true
      ));
  ...
  class Content {
    public static function all($options = array(), $type) {
      $options = array_merge(array('numberposts' => -1, 'post_type' => $type), $options);
  ...
  class Vacation extends Location {
    public static $post_type = "vacation";
    
    public static function all($options = array()) {
      return parent::all($options, "vacation");
    } 
 
    public static function visited($options = array()) {
      $options = array_merge(array('meta_key' => 'has_visited', 'meta_value' => 1, 'numberposts' => -1, 'post_type' => self::$post_type), $options);
      $raw_locations = get_posts( $options );
      $locations = array();
      foreach($raw_locations as $location) {
        $loc = new self($location);
    ...
    public static function non_visited($options = array()) {
      $options = array_merge(array('meta_key' => 'has_visited', 'meta_value' => 0, 'numberposts' => -1, 'post_type' => self::$post_type), $options);
      $raw_locations = get_posts( $options );
      $locations = array();
      foreach($raw_locations as $location) {
        $loc = new self($location);
  ...
  class Climb extends Content {
    public static $post_type = "climb";
    public static function all($options = array()) {
      return parent::all($options, "climb");
    } 
  }

  class Drive extends Content {
    public static $post_type = "drive";
    public static function all($options = array()) {
      return parent::all($options, "drive");
    } 
  }
